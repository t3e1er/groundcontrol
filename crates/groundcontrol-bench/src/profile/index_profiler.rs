...
impl IndexProfiler {
    ...
    pub fn profile(
    ...
    ) -> groundcontrol_common::Result<IndexingProfileReport> {
        ...
        let mut config: CorpusConfig = if config_path.exists() {
            let config_str = fs::read_to_string(&config_path).map_err(|e| {
                groundcontrol_common::Error::Config(format!(
                    "Failed to read groundcontrol.toml: {e}"
                ))
            })?;
        ...
        let disk =
            DiskProfiler::profile(corpus_dir).map_err(|e| groundcontrol_common::Error::Io(e))?;
