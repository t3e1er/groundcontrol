...
/// Load `groundcontrol.toml` from the corpus directory, or create a default config.
pub fn load_or_default_config(corpus_path: &Path) -> anyhow::Result<CorpusConfig> {
    let config_path = if corpus_path.join("groundcontrol.toml").exists() {
        corpus_path.join("groundcontrol.toml")
    } else {
        corpus_path.join("ctxvault.toml")
    };
    ...
    if config_path.exists() {
        ...
        Ok(CorpusConfig {
            ...
            graph: groundcontrol_common::config::GraphConfig {
                edge_types: vec![
                    groundcontrol_common::config::EdgeTypeConfig {
                        name: "Wikilink".to_string(),
                        source: groundcontrol_common::config::EdgeSource::Wikilink,
                        weight: 1.0,
                        bidirectional: false,
                        field: None,
                        direction: None,
                    ...
                    groundcontrol_common::config::EdgeTypeConfig {
                        ...
                        source: groundcontrol_common::config::EdgeSource::Tag,
                        weight: 0.5,
                        bidirectional: true,
                        field: None,
                        direction: None,
