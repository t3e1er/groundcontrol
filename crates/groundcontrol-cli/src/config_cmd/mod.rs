//! Configuration CLI command handlers for `groundcontrol config` (get/set/list).
//!
//! Backed by `${GROUNDCONTROL_CACHE_DIR}/config.toml`.

use groundcontrol_common::config::{
    get_config_path, load_global_config, save_global_config, IndexMode,
};

/// Handle `groundcontrol config list`.
...
/// Handle `groundcontrol config get <key>`.
