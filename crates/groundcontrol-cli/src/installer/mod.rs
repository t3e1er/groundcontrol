//! Auto-configuration and installation of groundcontrol for coding agents.
//!
//! Detects installed coding agents across platform standard paths, configures
//! `mcpServers` with a zero-arg `groundcontrol` launcher entry, and optionally
...
/// Default authoritative steering rule content for AI agents interacting with groundcontrol.
pub const GROUNDCONTROL_STEERING_RULE: &str = r#"# groundcontrol MCP Steering Protocol

You have access to the `groundcontrol` Model Context Protocol (MCP) server (17 authoritative tools). Follow these rules when querying or modifying knowledge and source code:
...
pub fn detect_rule_targets(workspace_dir: Option<&Path>) -> Vec<RuleTarget> {
    ...
    let antigravity_rules =
        home_path.join(".gemini").join("antigravity-ide").join("rules").join("groundcontrol.md");
    targets.push(RuleTarget { name: "Antigravity Global Rules", path: antigravity_rules });

    // 2. Gemini CLI global rules
    let gemini_rules =
        home_path.join(".gemini").join("config").join("rules").join("groundcontrol.md");
    targets.push(RuleTarget { name: "Gemini CLI Global Rules", path: gemini_rules });

    // 3. Cursor global rules
    let cursor_rules = home_path.join(".cursor").join("rules").join("groundcontrol.mdc");
    ...
    targets.push(RuleTarget {
        ...
        path: kiro_home.join("rules").join("groundcontrol.md"),
    });
    targets.push(RuleTarget {
        name: "Kiro CLI Global Steering",
        path: kiro_home.join("steering").join("groundcontrol.md"),
    });
    ...
    if let Some(ws) = workspace_dir {
        ...
        targets.push(RuleTarget {
            ...
            path: ws.join(".kiro").join("rules").join("groundcontrol.md"),
        });
        targets.push(RuleTarget {
            name: "Workspace Kiro Steering",
            path: ws.join(".kiro").join("steering").join("groundcontrol.md"),
        });
    }
...
pub fn run_install(
    ...
    let binary_command = if let Some(dir) = install_dir {
        let exe = if cfg!(windows) { "groundcontrol.exe" } else { "groundcontrol" };
        dir.join(exe).to_string_lossy().to_string()
    } else {
        "groundcontrol".to_string()
    };

    let mut clients_registry = if with_auth {
        let (reg, _) = groundcontrol_common::client::ensure_central_clients_config(true, false)?;
        Some(reg)
    } else {
        None
    };
    ...
    for target in targets {
        ...
        if let Some(ref mut reg) = clients_registry {
            ...
            server_entry.as_object_mut().unwrap().insert(
                "env".to_string(),
                json!({
                    "GROUNDCONTROL_API_KEY": key
                }),
            );
        }

        if let Some(servers_map) = servers.as_object_mut() {
            servers_map.insert("groundcontrol".to_string(), server_entry);
        }
        ...
        if dry_run {
            let auth_note =
                if with_auth { " (with GROUNDCONTROL_API_KEY credentials)" } else { "" };
            summary.dry_run_detected.push(format!(
                "{} -> Would configure groundcontrol{} at: {}",
    ...
    if install_rules {
        ...
        for rt in rule_targets {
            ...
            if dry_run {
                ...
                fs::write(&rt.path, GROUNDCONTROL_STEERING_RULE)?;
    ...
    for agents_dir in kiro_dirs {
        ...
        if let Some(ref mut reg) = clients_registry {
            ...
            let env_map = json!({ "GROUNDCONTROL_API_KEY": key });
        ...
        let scout_profile = json!({
            "name": "groundcontrol-scout",
            "description": "Fast exploratory code & doc scout agent using groundcontrol scout profile",
            "prompt": "You are a lightweight code scout. Use groundcontrol tools (search, get_snippet, status) for high-signal retrieval without full file dumps.",
            "tools": ["read", "grep", "glob"],
            "includeMcpJson": false,
            "mcpServers": {
                "groundcontrol": scout_server
            }
        });
        let analysis_profile = json!({
            "name": "groundcontrol-analysis",
            "description": "Deep architectural & graph analysis agent using groundcontrol analysis profile",
            "prompt": "You are an architectural analyst. Use groundcontrol graph_match, graph_communities, search, and validate for system mapping and dependency tracing.",
            "tools": ["read", "grep", "glob"],
            "includeMcpJson": false,
            "mcpServers": {
                "groundcontrol": analysis_server
            }
        });

        let scout_path = agents_dir.join("groundcontrol-scout.json");
        let analysis_path = agents_dir.join("groundcontrol-analysis.json");
    ...
    if let Some(ref reg) = clients_registry {
        if !dry_run {
            let mut global_cfg = groundcontrol_common::config::load_global_config();
            global_cfg.auth = reg.clone();
            let _ = groundcontrol_common::config::save_global_config(&global_cfg);
            let central_path = groundcontrol_common::client::get_central_clients_path();
            let _ = groundcontrol_common::client::save_clients_config(reg, &central_path);
        }
    }
...
mod tests {
    ...
    fn test_kiro_installer_auto_configuration() {
        ...
        assert!(
            mcp_content["mcpServers"]["groundcontrol"]["command"].is_string(),
            "groundcontrol server command should be configured in mcpServers"
        );

        // 2. Verify Kiro subagent profiles were generated
        let scout_agent_path = ws_kiro.join("agents").join("groundcontrol-scout.json");
        let analysis_agent_path = ws_kiro.join("agents").join("groundcontrol-analysis.json");
        assert!(scout_agent_path.exists(), "groundcontrol-scout.json should be created");
        assert!(analysis_agent_path.exists(), "groundcontrol-analysis.json should be created");

        let scout_agent: Value =
            serde_json::from_str(&fs::read_to_string(&scout_agent_path).unwrap()).unwrap();
        assert_eq!(scout_agent["name"], "groundcontrol-scout");
        let scout_args = scout_agent["mcpServers"]["groundcontrol"]["args"].as_array().unwrap();
        assert_eq!(scout_args, &vec![json!("--profile"), json!("scout")]);

        // 3. Verify Kiro steering rule was installed
        let rules_path = ws_kiro.join("rules").join("groundcontrol.md");
        let steering_path = ws_kiro.join("steering").join("groundcontrol.md");
        assert!(rules_path.exists(), "rules/groundcontrol.md should be written");
        assert!(steering_path.exists(), "steering/groundcontrol.md should be written");
        assert!(fs::read_to_string(&rules_path)
            .unwrap()
            .contains("groundcontrol MCP Steering Protocol"));
    ...
    fn test_installer_with_auth_populates_credentials() {
        ...
        let env_key =
            mcp_content["mcpServers"]["groundcontrol"]["env"]["GROUNDCONTROL_API_KEY"].as_str();
        assert!(
            env_key.is_some(),
            "GROUNDCONTROL_API_KEY must be populated in mcpServers.groundcontrol.env"
        );
