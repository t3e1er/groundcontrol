//! Ports for the groundcontrol hexagonal (ports-and-adapters) architecture.
//!
//! # Hexagonal intent
//!
//! groundcontrol is organized as **ports and adapters**. A *port* is a contract —
...
//! live in `groundcontrol-core`, implement these ports, and keep their backend types
//! (`rusqlite::Connection`, `tantivy::*`, `hnsw_rs::*`, `petgraph::*`, `ort::*`)
//! encapsulated: a backend type must never leak across a port boundary. The
//! **composition root** in `groundcontrol-cli` is the only place that names concrete
...
//! six port traits live here in `groundcontrol-common::ports`**:
//!
//! - **`MetadataCatalog`** — in `groundcontrol-common`. The SQLite `Store`'s public
...
//! - **`TextIndex`** — in `groundcontrol-common`. The Tantivy `BM25Index`'s used
...
//! - **`VectorStore`** — in `groundcontrol-common`. The HNSW index's used surface
...
//! - **`GraphStore`** — in `groundcontrol-common`. The Petgraph `KnowledgeGraph`'s
...
//! - **`EmbeddingProvider`** — in `groundcontrol-common`. The `Embedder`'s used
//!   surface takes `&str`/`&[&str]` and returns `Vec<f32>` / `Vec<Vec<f32>>`;
//!   `ort::*` and tokenizer types stay internal.
//! - **`SearchService`** — in `groundcontrol-common`. Search-mode dispatch and RRF
