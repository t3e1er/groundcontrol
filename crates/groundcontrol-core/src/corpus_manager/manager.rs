...
impl CorpusManager {
    ...
    pub fn ensure_corpus_with_name(
        ...
        // Index storage: always central storage in ${GROUNDCONTROL_CACHE_DIR}/corpora/<name>
        let local_config = if canonical.join("groundcontrol.toml").exists() {
            canonical.join("groundcontrol.toml")
        } else {
            canonical.join("ctxvault.toml")
        };
    ...
    pub fn mount_all_cached_corpora(&mut self) -> Result<Vec<String>> {
        let cache_dir = groundcontrol_common::config::get_corpora_cache_dir();
    ...
    pub fn import_corpus(
        ...
        let config = CorpusConfig {
            ...
            chunking: groundcontrol_common::config::ChunkingConfig::default(),
            embedding: groundcontrol_common::config::EmbeddingConfig {
