...
fn test_config(corpus_path: &Path) -> CorpusConfig {
    CorpusConfig {
        ...
        chunking: groundcontrol_common::config::ChunkingConfig {
        ...
        graph: groundcontrol_common::config::GraphConfig {
            edge_types: vec![groundcontrol_common::config::EdgeTypeConfig {
...
fn test_full_mode_docs_vector_code_hamming() {
    ...
    let mut config = test_config(&corpus_dir);
    config.index_mode = groundcontrol_common::config::IndexMode::Full;
    ...
    let has_anchor = doc_pending
        .iter()
        .any(|c| c.embed_policy == groundcontrol_common::types::ChunkEmbedPolicy::Anchor);
