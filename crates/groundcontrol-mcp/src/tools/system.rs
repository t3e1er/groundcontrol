...
fn apply_index_mode_override(
    ...
    if let Some(mode_str) = index_mode {
        match mode_str.to_lowercase().as_str() {
            "fast" => engine.set_index_mode(groundcontrol_common::config::IndexMode::Fast),
            "full" => engine.set_index_mode(groundcontrol_common::config::IndexMode::Full),
...
pub fn handle_list_corpora_manager(manager: &CorpusManager, args: Value) -> Result<Value> {
    ...
    let mut corpora_info: Vec<Value> = loaded
        .into_iter()
        .map(|c| {
            let index_path = groundcontrol_common::config::get_corpus_index_dir(&c.name);
            serde_json::json!({
                "name": c.name,
                "path": c.path,
    ...
    if include_cached {
        for cached in manager.discover_cached_corpora() {
            if !loaded_names.contains(&cached) {
                let cache_dir = groundcontrol_common::config::get_corpus_index_dir(&cached);
                let source_path =
                    groundcontrol_core::corpus_manager::CorpusManager::get_cached_corpus_source_path(
                        &cached,
                    )
